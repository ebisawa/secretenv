...
2. **信頼境界**: ローカル秘密鍵とローカルキーストアは信頼し、workspace 上の公開鍵・暗号ファイルは信頼しない
3. **鍵の本人性の限界**: 自己署名と attestation は鍵一貫性や鍵紐付けを示すが、本人性は単独の機構で確定できない。初回受理確認（昇格時の人手承認）や運用判断は、本人性判断のための材料を増やす層である
4. **文脈束縛**: `sid` / `kid` / `k` / `p` を使って流用や取り違えを防いでいる
5. **実装上の重要な不変条件**: 署名検証前に復号しないこと、binding を削っていないこと、環境変数経由の鍵ロードの制約を守ること
...
| **文脈束縛**            | `sid` / `kid` / `k` / `p` を info / AAD に含める | `sid` / `kid` / `k` / `p` を HPKE info と payload AAD に束縛し、流用・入れ替えが成立しないようにする | 実装が仕様どおりの binding を維持する  | binding を削る実装変更で弱くなる             |
| **鍵一貫性**            | PublicKey 自己署名                              | PublicKey 本体を自己署名で保護し、既存鍵の改ざんが成立しないようにする                                    | 元の秘密鍵が漏洩していない            | 新規の悪意ある鍵作成は防げない                  |
| **鍵の本人性補強**         | SSH attestation + 初回受理確認 + online verify    | 各層の意味を分離して扱い、混同しない（自己署名=一貫性、attestation=紐付け、初回受理確認=昇格時の人手判断）                | 初回受理確認が適切に行われる           | `--force` や誤承認で弱くなる              |
| **Portable な秘密鍵利用** | password export または SSH ベース保護               | CI 利用時の trust condition を満たす運用に限定する                                         | trusted CI context でのみ使う | 同一 secret backend への保存は独立防御にならない |
...
この前提が満たされない環境では、incoming → active の昇格プロセスに加え、リポジトリ層でのアクセス制御を別途実施する必要がある。SecretEnv 自体の暗号設計と、配布媒体のアクセス制御は分けて評価する必要がある。
...
| **実装で最重要な観点**   | 署名検証順序、binding の保持、環境変数経由の鍵ロードの制約、PublicKey 検証の省略禁止    |
...
**層3: 初回受理確認（鍵と人物の対応付けに関する判断材料）**

`rewrap` を実行するユーザーが、incoming メンバーの SSH fingerprint と GitHub アカウント情報を目視で確認する。SSH の `known_hosts` における初回接続確認と同等の信頼モデルである。**これは本人性を確定するものではなく、「この鍵を誰の鍵として受け入れるか」を判断するための材料を提供する。**

**`--force` による省略のリスクと推奨運用**:
`--force` を指定すると対話的な確認をスキップして昇格が行われるため、本人性判断の材料が減り、公開鍵すり替え攻撃に対する最終防御層が弱まる。ただし、online 検証（層4）が実施され明示的に失敗したメンバーは `--force` 使用時であっても昇格対象から除外される。

自動化スクリプトなど非対話環境での `rewrap` には `--force` の使用が必要になる場合があるが、その場合は以下の運用を推奨する:
- 可能な限り、新メンバーの昇格は対話環境の `rewrap` で事前に完了させておく
- `--force` 使用後は `member verify` で online 検証を実施し、昇格されたメンバーの正当性を事後確認する
- `--force` の使用はチーム内で運用ポリシーとして管理し、日常的な手動操作での無制限な使用を避ける
...
- **新規鍵挿入**: 初回受理確認の誤承認（または `--force` による省略）のみで成立する。攻撃者は自分の鍵で有効な自己署名・attestation を生成できるため、被害者の SSH 鍵漏洩は不要
...
各行は LF (`0x0A`) で区切る。`member_id` は任意文字列のため暗号学的用途には使用せず、canonical な `kid` のみを使用する。
...
2. ローカルキーストアからロードする場合は、同じ `kid` ディレクトリの `public.json` も読み込み、PublicKey 検証と `member_id` / `kid` 整合確認を行う
3. `protected` から `kid`、`salt`、SSH 鍵 fingerprint を取得する
4. `kid + salt` から署名メッセージを構築する
5. SSH 鍵で署名し、raw Ed25519 署名値を IKM として抽出する
6. HKDF で `enc_key` を導出する
...
HKDF info 文字列（`secretenv:password-private-key-enc@4:{kid}`）は SSH ベースのスキーム（`secretenv:private-key-enc@4:{kid}`）とは異なり、2 つの鍵導出パスのドメイン分離を保証する。
...
### 8.4 署名検証で参照される公開鍵とメンバーシップ確認

署名検証に用いる公開鍵（verification key）の取得元と、その鍵が「正当なメンバーのものであるか」の確認方法は以下の通り整理される。

**1. 公開鍵の取得元**
- **`signer_pub` が埋め込まれている場合**: その埋め込み PublicKey を検証鍵として使用する。
- **`signer_pub` がない場合**: `signature.kid` を用いて workspace の `members/active/` から PublicKey を検索して使用する。

（注: ローカルキーストアは秘密鍵の保管用であり、署名検証の公開鍵ソースとしては決して参照しない。）

**2. 埋め込み公開鍵の自己検証**
`signer_pub` が埋め込まれている場合、その PublicKey 文書自体が正当であることをまず検証する。
- 自己署名、有効期限、`kid` の一致、および attestation を検証する。
- `kid` は単なる参照ラベルではなく、文書内容（`protected_without_kid`）から一意に導出される。そのため、再導出した `kid` が宣言された `kid` と一致することを確認し、内容の改ざんがないことを担保する。

**3. メンバーシップの確認**
検証鍵が正当な暗号鍵であっても、それが「現在の workspace の active メンバーであるか」を別途確認する必要がある。
- これは、**workspace の `members/active/` に同じ `kid` が存在すること**で確認する。
- `kid` の一致は鍵ステートメントの完全一致を意味するため（§4.3.1）、`kid` の存在チェックのみで十分であり、ファイル全体のバイト単位の比較は必要としない。

**4. 信頼の境界（Workspace の位置づけ）**
workspace の `active` ディレクトリは「どの鍵が現在有効とみなされているか」のリストを提供するが、それ自体が**絶対的な trust anchor ではない**。
- 前述の通り、どの鍵を「誰の鍵として受け入れるか」の最終的な本人性判断は、各ユーザーが自己の責任で行う（§2.5 層3 初回受理確認）。
- その判断を支援するための情報として、SSH attestation や GitHub による online verify などの証拠が提供される。
...
**実装上の注意:** 上表の各 binding point は削ってはならず、別の入力値に置き換えてはならない。比較は文字列ではなく canonicalized bytes で行わなければならない。
...
| **主要防御**             | (1) 初回受理確認 (2) online verify による補助情報                      |
| **弱くなる条件**           | 初回受理確認の誤承認、`--force` による省略、GitHub アカウント侵害                 |
| **期待される失敗点**         | 人手の拒否または verification failure による昇格拒否                     |


**重要**: 自己署名は既存 PublicKey の改竄を防ぐが、攻撃者が自分の鍵で正規の手順に従って新規 PublicKey を作成することは防げない。新規鍵挿入に対する最終防御は初回受理確認（§2.5 層3）である。`--force` による初回受理確認の省略は、この防御を意図的に無効化するものであり、使用には十分な注意が必要である。
...
これらのシナリオには共通の構造がある。文脈束縛（`sid`, `kid`, `k`, `p`）と Ed25519 署名が 2 つの独立した防御層を形成している。両方を同時に迂回するには、署名者の秘密鍵の漏洩か、binding の削除・処理順序の変更といった実装上の欠陥が必要となる。§11 の検証項目はこのような欠陥の検出を目的としている。
...
| **環境変数経由の鍵ロード**     | trusted CI context でのみ利用し、鍵ロード時に self PublicKey の workspace lookup を行わない | attacker-controlled checkout で秘密鍵を誤用し得る |
...
**設計意図:** SecretEnv はシステム全体として強い意味での forward secrecy を主張しない。`--rotate-key` は漏洩後の被害限定措置であり、過去データの保護を取り戻す仕組みではない。
...
graph TB
    ...
    subgraph private_key["PrivateKey (ローカルキーストア)"]
        PK_ENC["secretenv.private.key@4<br/>SSH 署名ベース暗号化"]
    end
...
この図は、「どの秘密鍵がどのオブジェクトを保護し、どこで署名や wrap が使われるか」を俯瞰するための補助資料である。個々の binding point や検証順序は本文の §9 と §11 を優先して参照する。