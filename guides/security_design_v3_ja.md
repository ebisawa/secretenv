...
- パラメータは秘密鍵文書の `alg` オブジェクトに記録され、復号時にそこから読み取られる（ハードコードではない）
- 復号時の最小パラメータ検証: m >= 19456 (19 MiB), t >= 1, p >= 1（極端に弱いパラメータを拒否）
- パスワードの最小長: 8 文字
- 将来のパラメータ変更は前方互換: 既存の鍵は記録済みパラメータを保持する
...
代表的な実装手段として、秘密鍵や復号後平文は zeroizing wrapper によりスコープ終了時に消去する。
