...
- **暗号化/更新**（`encrypt`, `set`, `unset`, `import`）: 環境変数の署名鍵を使用し、受信者の公開鍵は workspace から取得
- **Rewrap**（`rewrap`）: 環境変数の署名鍵を使用し、公開鍵は workspace から取得

以下のローカルキーストア/SSH signer 前提コマンドは CI の環境変数モードではサポートされません。
