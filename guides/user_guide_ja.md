...
`rewrap` で incoming メンバーを active に昇格させるとき、secretenv は対話的な確認を行います。画面に GitHub のログイン名、ユーザー ID、SSH 鍵のフィンガープリントが表示され、実行者が「この鍵を誰の鍵として受け入れるか」を `y` / `N` で判断します（本人性を暗号学的に確定するものではありません）。
...
この確認が重要な理由は、悪意のある第三者が本物のメンバーになりすまして PR を出した場合に「見覚えのない GitHub アカウントや SSH 鍵が表示される」ことで不正を検知できるからです。`--force` を指定すると初回受理確認がスキップされるため、このセーフガードが機能しなくなります。
...
| 層3 | 初回受理確認 | 鍵と人物の対応付けに関する判断材料（目視確認） | `--force` 使用時は省略される |
...
#    - 初回受理確認（表示された鍵情報を目視確認）
secretenv rewrap

# 初回受理確認の例（3章で説明した確認プロンプト）:
# Member bob@example.com
#   GitHub login: bob-gh (id: 12345678)
#   SSH key fingerprint: SHA256:xxxxx...
...
ここで重要なのは、workspace checkout は依然として署名検証の入力であり trust boundary 外だという点です。したがって、環境変数による鍵読み込みは **trusted workflow / trusted ref / trusted runner** を満たす job でのみ使ってください。
...
### 自動化スクリプト等での利用と `--force` のリスク

自動化スクリプトなど非対話環境で `rewrap` 等を実行するために `--force` が必要な場合は、以下のリスクに注意してください。
（注: CI/CD 環境での推奨構成である「環境変数経由の鍵ロード」では、そもそも `rewrap` などの更新系コマンドはサポートされていません。詳細は[12章: CI/CD 連携](#12-cicd-連携) を参照してください。）

**`--force` のリスク**: 3章で説明した通り、初回受理確認は公開鍵すり替え攻撃に対する「最後の防御層」です。`--force` でこれをスキップすると、不正な公開鍵が紛れ込んでも気付けない可能性があります。

非対話環境で `--force` を安全に使うためのルール:

1. **新メンバーの承認は対話環境で先に完了させる**: 可能な限り、対話端末で `rewrap` を実行して incoming メンバーを active に昇格させておく
2. **`--force` 後は事後確認する**: `secretenv member verify` で全メンバーの GitHub 照合を実施する
3. **`--force` の使用を限定する**: `--force` は自動化パイプラインなど限られた場所でのみ使用し、日常の対話操作では使わない
